...
public abstract class HttpDecorator<REQUEST, RESPONSE> {
  protected final FSOConfiguration FSO_CONFIGURATION = FSOConfiguration.ConfigProvider.get();
  ...
  /** Returns the new span name according to FSO specs */
  ...
  public void decorateRequest(Span span, REQUEST request) {
    if (!FSO_CONFIGURATION.metadataOnly()) {
      span.setAttribute(
          FSOSemanticAttributes.HTTP_REQUEST_HEADERS,
          getFilteredRequestHeaders(request).toString());
    }
    URI url = getUrl(request);

    if (url != null) {
      span.setAttribute(FSOSemanticAttributes.HTTP_REQUEST_PATH, url.getPath());
      span.setAttribute(FSOSemanticAttributes.HTTP_REQUEST_QUERY, url.getQuery());
    }
  }
  ...
  public void decorateResponse(Span span, RESPONSE response) {
    if (!FSO_CONFIGURATION.metadataOnly()) {
      span.setAttribute(
          FSOSemanticAttributes.HTTP_RESPONSE_HEADERS,
