...
package com.fso.agent.core.instrumentation.utils;
...
import com.fso.agent.core.instrumentation.FSOSemanticAttributes;
...
public final class HeadersHandler {
  ...
  private static final FSOConfiguration fsoConfiguration = FSOConfiguration.ConfigProvider.get();

  public static void addRequestHeaders(
      Span span, Iterator<?> headerIterator, Class<?> headerClass) {
    addHeaders(span, headerIterator, headerClass, FSOSemanticAttributes.HTTP_REQUEST_HEADERS);
  }

  public static void addRequestHeaders(Span span, Map<String, String> headersMap) {
    addHeaders(span, headersMap, FSOSemanticAttributes.HTTP_REQUEST_HEADERS);
  }

  public static void addResponseHeaders(
      Span span, Iterator<?> headerIterator, Class<?> headerClass) {
    addHeaders(span, headerIterator, headerClass, FSOSemanticAttributes.HTTP_RESPONSE_HEADERS);
  }

  public static void addResponseHeaders(Span span, Map<String, String> headersMap) {
    addHeaders(span, headersMap, FSOSemanticAttributes.HTTP_RESPONSE_HEADERS);
  }
